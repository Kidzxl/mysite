...
class RegForm(forms.Form):
    username =forms.CharField(label="用户名",max_length=30,
                              ...
                              widget=forms.TextInput(
                                    attrs={'class':"form-control",'placeholder':'请输入3-30用户名'}))
    password =forms.CharField(label="密码",max_length=30,
                              min_length=6,
                              widget=forms.PasswordInput(
                                    attrs={'class':"form-control",'placeholder':'请输入密码'}))
    password_again =forms.CharField(label="密码",max_length=30,
                              min_length=6,
                              widget=forms.PasswordInput(
    ...
    verification_code = forms.CharField(
                            label="验证码",
                            required=False,
                            widget=forms.TextInput(attrs={'class': "form-control", 'placeholder': '点击“发送验证码”到邮箱'})
    )
    ...
    def clean_username(self):
        usernmae = self.cleaned_data['username']
    ...
    def clean_verfication_code(self):
...
class ChangeNicknameForm(forms.Form):
    ...
    def clean_nickname_new(self):
        ...
        if nickname_new == ''or nickname_new == ' ':
...
class ChangePasswordForm(forms.Form):
    ...
    new_password_again=forms.CharField(label="再次输入新的密码",
        widget=forms.PasswordInput(
                attrs={'class':"form-control",'placeholder':'再次请输入新的密码'
        }))
    def __init__(self,*args,**kwargs):
        if 'user' in kwargs :
    ...
    def clean(self):        #验证新的密码是否一致
        new_password = self.cleaned_data.get('new_password','')
        new_password_again = self.cleaned_data.get('new_password_again','')
        if new_password != new_password_again or new_password=='':
            raise ValidationError("两次密码不一致或密码为空")
        return self.cleaned_data

    def clean_old_passowrd(self):
        #验证旧的密码是否正确
        old_password = self.user.cleaned_data.get('old_password','')
        if not old_password.check_password(old_password):
...
class ForgotPasswordForm(forms.Form):
    ...
    def clean_verfication_code(self):
        ...
        code = self.request.session.get("forgot_password", "").lower()
        verification_code = self.cleaned_data.get("verification_code", "").lower()
        if not (code != "" and code == verification_code):
            raise ValidationError("验证码不正确")
