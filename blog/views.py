...
def get_blog_list_common_data(blogs_all,request):
    paginator = Paginator(blogs_all, settings.EACH_PAGE_BLOGS_NUMBER)  # 每十页进行分页
    page_num = request.GET.get("page", 1)  # 获取页码参数
    page_of_blogs = paginator.get_page(page_num)
    currentr_page_num = page_of_blogs.number  # 获取当前页码
    page_range = list(range(max(1, currentr_page_num - 2), currentr_page_num)) + \
                 list(range(currentr_page_num, min(paginator.num_pages, currentr_page_num + 2) + 1))
    # 加上省略页码标记
    if page_range[0] - 1 >= 2:
        page_range.insert(0, "...")
    ...
    context['blog_dates'] =blog_dates_dict # 得到日期列表
    return context

def blog_list(request):
    blogs_all = Blog.objects.all()
    context = get_blog_list_common_data(blogs_all,request)
...
def blog_detail(request,blog_pk):
    ...
    response.set_cookie(read_cookie_key,'true', )#expires = detetime max_age=60
    return response
